...
from source.models import load_model
...
def train_loop(model, clip_model, clip_processor, optimizer, dataset):
    model.train()

    volume, tangent, text = dataset.pop_data_text()
    slicer = Slicer(im=volume, out_size=dataset.plane_size, device=model.model_device)
    inputs = clip_processor([text], return_tensors="pt")
    with torch.no_grad():
        text_embeddings = clip_model.get_text_features(**inputs)
    
    fwd = model(tangent, text_embeddings, slicer, dataset.batch_size)
    optimizer.zero_grad()
...
def val_loop(model, clip_model, clip_processor, dataset, sess, iteration):
    model.eval()
    recorder = Recorder()
    for idx in range(dataset.num):
        volume, tangent, _, text = dataset.pop_data_text_idx(idx)
        slicer = Slicer(im=volume, out_size=dataset.plane_size, device=model.model_device)
        ...
        with torch.no_grad():
            eval_dict = model.evaluate(tangent, text_embeddings, slicer, dataset.batch_size)
...
def main(cfg):
    ...
    clip_model = AutoModel.from_pretrained("./BiomedCLIP", trust_remote_code=True)
    clip_processor = AutoProcessor.from_pretrained("./BiomedCLIP", trust_remote_code=True)
    ...
    for iteration in range(cfg['NumIterations']):
        loss_info, print_info = train_loop(model, clip_model, clip_processor, optimizer, train_set)
        session.log(loss_info, step=iteration)
        info = f'Iteration {iteration:05d} >>> {print_info}'
        print(info)
        if iteration % cfg['ValFreq'] == 0:

            val_info, val_metric = val_loop(model, clip_model, clip_processor, val_set, session, iteration)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--config_path", default='./configure/Prototyping_clip.yaml')
